 ...
 * @author kbnc
 */
public class ReadEmployeeAllHistoryG extends HttpServlet {

   Employee gupp;
    @Override
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        
        System.out.println("loadusers_all_servlet_call_g");
        
         Session sess = NewHibernateUtil.getSessionFactory().openSession();
        try {
            
            
              String nic=(String) req.getSession(false).getAttribute("nic");
        ...
        GeneralUserProfile gup1 = (GeneralUserProfile) sess.createQuery("From GeneralUserProfile Where nic='" + nic + "'").setMaxResults(1).uniqueResult();
        
        Criteria c=sess.createCriteria(Employee.class);
        c.add(Restrictions.eq("generalUserProfile", gup1));
...
//        Employee emp = (Employee) sess.createQuery("From Employee Where epfNo='" + employee.getEpfNo() + "'").setMaxResults(1).uniqueResult();
            ...
            List<WorkHistory> list = sess.createQuery("From WorkHistory Where employee='"+employee.getId()+"'").list();
            if (!list.isEmpty()) {
